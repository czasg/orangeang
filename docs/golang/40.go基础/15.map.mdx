...
在切片中，我们了解了`make`的使用，在 Map 中，`make`同样用于初始化，不过二者仍有一定区别：

- `make([]int, 0, 100)`：最多支持两个参数，分别表示切片的长度和容量。
- `make(map[int]int, 100)`：最多支持一个参数，表示Map的空间大小。

如何理解 Map 的空间大小？

Map 拥有扩容因子，当集合元素达到一定阈值时，就会触发扩容。因此通过`make`可以指定初始空间大小，避免高频的触发扩容。
...
- 当 key 不存在时：返回对应数据类型的零值。

因此，我们无法通过 Map 的返回值来判断 key 是否存在，因为总是有返回值。

于是，Go 在 Map 查询操作时，额外返回了一个布尔类型的值，用于判断 key 是否存在。
