...
我们与其他团队交接了一个服务，结果发现该服务所在的服务器每天凌晨 3 点到 4 点之间，磁盘空间会突然被占满，随后又自动恢复。

初步判断，这可能是由于某个定时任务在执行过程中生成了大量临时文件，任务完成后又自动删除了这些文件。

然而，令人困惑的是，代码中并没有设置凌晨 3 点到 4 点的定时任务，服务器上的 crontab 也没有相关记录...

<!--truncate-->

经过初步排查，我们确认问题出在服务中的某个“幽灵”程序，该程序会在凌晨异常执行，导致磁盘空间被占满。初步推测，该程序可能向 `/tmp` 或 `/var` 目录写入了大量数据。

为了进一步排查问题，我们编写了一个磁盘检查脚本，当磁盘利用率超过 70% 时自动触发，并生成详细的日志文件，帮助定位问题。脚本的整体逻辑如下：
